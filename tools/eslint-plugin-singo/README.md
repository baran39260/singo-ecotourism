...
| نام                                                                        | severity پیشنهادی | توضیح                                                                      |
| -------------------------------------------------------------------------- | ----------------- | -------------------------------------------------------------------------- |
| [`singo/no-core-internal-leak`](rules/no-core-internal-leak.mjs)           | `error`           | فایل‌های درون `src/core/` نباید از `src/overrides/` import کنند            |
| [`singo/overrides-stable-api`](rules/overrides-stable-api.mjs)             | `warn`            | فایل‌های `src/overrides/` فقط از سطح بالای core import کنند، نه مسیر داخلی |
| [`singo/no-feature-internal-import`](rules/no-feature-internal-import.mjs) | `error`           | به feature فقط از `index.ts` آن import شود، نه مسیر داخلی                  |
